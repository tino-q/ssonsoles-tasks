...
const API_BASE_URL = process.env.VITE_API_URL || 'MISSING_API_URL';
...
class ApiService {
  ...
  async updateTaskStatus(taskId, status, comments = '') {
    return this.request('updateTaskStatus', { data: { taskId, status, comments } });
  }
  ...
  async requestProducts(taskId, productRequests) {
    return this.request('requestProducts', { data: { taskId, productRequests } });
  }

  // Reports
  async getMonthlyReport(month, year, cleanerId = null) {
    return this.request('getMonthlyReport', { data: { month, year, cleanerId } });
  }
