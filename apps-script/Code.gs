...
function doPost(e) {
  ...
  try {
    switch(action) {
      ...
      case 'assignTask':
        return createResponse(TaskService.assignTask(data.taskId, data.cleanerId));
      case 'updateTaskStatus':
        return createResponse(TaskService.updateTaskStatus(data.taskId, data.status, data.comments));
      case 'requestProducts':
        return createResponse(ProductService.requestProducts(data.taskId, data.productRequests));
      case 'importReservations':
        return createResponse(TaskService.importReservations());
      case 'getMonthlyReport':
        return createResponse(ReportService.getMonthlyReport(data.month, data.year, data.cleanerId));
      default:
        return createResponse({ error: 'Invalid action' }, 400);
    }
  } catch (error) {
...
function testSetup() {
  try {
    ...
    const sheets = ['reservations', 'tasks', 'cleaners', 'products', 'product_requests'];
    sheets.forEach(sheetName => {
      const sheet = getSheet(sheetName);
      if (sheet) {
        console.log(`Sheet '${sheetName}' found with ${sheet.getLastRow()} rows`);
      } else {
