...
const ReportService = {
  ...
  getPropertyReport: function(startDate = null, endDate = null) {
    ...
    tasks.forEach(task => {
      ...
      if (task.cleaner) {
        stats.cleaners_used.add(task.cleaner);
      }
    });
    ...
    Object.values(propertyStats).forEach(stats => {
      ...
      stats.average_hours = stats.completed_tasks > 0 ? 
        Math.round((stats.total_hours / stats.completed_tasks) * 100) / 100 : 0;
      stats.total_hours = Math.round(stats.total_hours * 100) / 100;
    });
    
    return Object.values(propertyStats).sort((a, b) => b.total_tasks - a.total_tasks);
  },
  ...
  getSystemReport: function(startDate = null, endDate = null) {
    ...
    const report = {
      ...
      product_summary: ProductService.getProductRequestSummary(startDate, endDate)
    };
    ...
    filteredTasks.forEach(task => {
      ...
      if (task.cleaner) {
        const cleaner = cleaners.find(c => c.id == task.cleaner);
        if (cleaner && hours > 0) {
    ...
    // Round totals
    report.totals.total_hours = Math.round(report.totals.total_hours * 100) / 100;
    report.totals.total_payment = Math.round(report.totals.total_payment * 100) / 100;
